				...
				<view class="notice-item" @tap="todetail(JSON.stringify(item))">
					...
					<view class="content">
						<view class="top">
							...
							<text class="redround" :class="item.isRead==0?'redcolor':'greycolor'">{{item.isRead==0?'未读':'已读'}}</text>
						</view>
...
<script>
	...
	export default {
		...
		async onShow() {
			let that = this;
			if(that.hasLogin){
				let sysmsglist = await this.$request.post('/user/getUseSystemMessage', {
					data:{
						"base": {

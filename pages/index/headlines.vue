	...
	export default {
		...
		data() {
			return {
				...
				pageNo:1,
				pageSize:10,
				navTitle: this.$t('headlines.index1')
			}
		},
		onLoad() {
		...
		async onShow() {
			...
			let that = this;
			if(that.hasLogin){
				let sysmsglist = await this.$request.post('/uc/announcement/page', {
					header: {
				...
				if(sysmsglist.data.data.content.length > 0){
					that.$data.noticelist = sysmsglist.data.data.content;
					that.$data.nlshow = "";
				}else{
					that.$data.nlshow = this.$t('headlines.index1')+"~";
				}
			}
		},
		methods: {

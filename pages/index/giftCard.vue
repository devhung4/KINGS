<template>
  <div class="transfer_box">
    <div class="navBox">
      ...
      <div class="title">{{ $t('index.giftCard.pageTitle') }}</div>
      <image
        class="rec"
    ...
    <div class="bld">
      <div v-if="!success">
        ...
        <div class="card_box">
          <div class="hed_1">{{ $t('index.giftCard.choiceCard') }}</div>
          <div class="hed_2">
            <div
          ...
          <div></div>
          <div class="hed_3">2、額外給好友加個紅包</div>
          ...
          <div class="hed_5">
            <input
              ...
              inputmode="numeric"
              v-model="amount"
              :placeholder="$t('index.giftCard.amount.inputCustomAmount')" />
          </div>
        ...
        <view class="apply_btn">
          <div class="shenqin">
            <div class="lef">
              <div>{{ $t('index.giftCard.paymentAmount') }}</div>
              <div>{{ buyPrice }} USDT</div>
            </div>
            <div
              class="rig"
              @tap="$refs.buyNowFormRef.open()">
              {{ $t('index.giftCard.buyNow') }}
            </div>
            <!-- 测试按钮 -->
            <!-- <button @tap="$refs.buyNowFormRef.open()">Buy Now</button> -->
          </div>
        </view>
      </div>
    ...
    <uni-popup
      ...
      type="center">
      <view class="input-password-form">
        <view class="poup_hed">
          <view>资金密码</view>
          <!-- <image
            src="../../static/image/clor.png"
            @click="pasPoup = false" /> -->
          <view
            @tap="$refs.inputPasswordRef.close()"
            class="close"></view>
        </view>
        <view class="poup_bld">
          <div class="poup_tex">
    ...
    <template>
      <uni-popup
        ref="buyNowFormRef"
        type="bottom">
        <view class="buy-now-form">
          <view class="buy-now-form__head">
            <text class="buy-now-form__head-title">確認支付</text>
            <view
              @tap="$refs.buyNowFormRef.close()"
              class="buy-now-form__head-close">
              <!-- <image
                src="../../static/icons/icons8-close-200.png"
                mode="scaleToFill"
              /> -->
            </view>
          </view>
          <view class="buy-now-form__content">
            <form class="buy-now-form__content-form">
              <view class="buy-now-form__content-form-body">
                <view class="item">
                  <text>卡片類型</text>
                  <text>{{ cardName }}</text>
                </view>
                ...
                <view class="item">
                  <text>支付金額</text>
                  <text>{{ Number(amount) + Number(buyPrice) }}</text>
                </view>
                <view class="item">
                  <text>賬戶餘額</text>
                  <text>{{ Math.floor(price) }}</text>
                </view>
              </view>
              <view class="buy-now-form__content-form-button-group">
                <button
                  @tap="$refs.buyNowFormRef.close()"
                  class="cancel">
                  取消
  ...
  methods: {
    ...
    async buy() {
      ...
      if (!this.amount + this.buyPrice > this.price) {
        this.$api.msg('余额不足')
        this.$refs.buyNowFormRef.close()
        return
      }
      this.$refs.buyNowFormRef.close()
      // this.applyShow = false
      // this.pasPoup = true
      this.$refs.inputPasswordRef.open()
    },
    ...
    async openCard() {
      ...
      if (res.data.code == 0) {
        ...
        this.$refs.inputPasswordRef.close()
      } else {
...
.buy-now-form {
  ...
  &__head {
    ...
    &-close {
      ...
      justify-content: center;
      align-items: center;
      background: url('../../static/image/icons8-close-200.png') center center /
        24px 24px no-repeat;
      width: 24px;
      height: 24px;
    }
  ...
  &__content {
    ...
    &-form {
      ...
      &-body {
        ...
        width: 100%;
        min-width: 100%;
        .item:first-child text:last-child {
          color: #f0c473;
        }
        .item:nth-child(2) text:last-child {
          color: #f26162;
        }
        .item {
          display: flex;

		...
		<view class="content">
			<view class="golden_info">
				...
				<view class="golden_num">
					<text>{{goldeninfo.activity}}</text>个
				</view>
				<view class="profit">
					<text>今日收益</text>
					<text class="add">+{{goldeninfo.todayActivity}}</text>
					个
				</view>
			</view>
			<view class="detailed">
				<view class="video">
					<view class="video_title">
						<text class="audt">活跃度明细</text>
					</view>
				</view>
				<view class="det" v-if="activityinfo.length>0">
					<view class="detailed_list" v-for="(item,index) in activityinfo" :key="index">
						<view class="det_top">
...
<script>
	export default {
		...
		methods: {
			...
			//活跃度变动记录
			async getactivity() {
				let activity = await this.$request.post('/energy/axis/activity', {
					"data": {
							"uid": this.uid,
							"sequence": 0
				...
				this.activityinfo = activity.data.data;
...
<style lang='scss'>
	...
	.golden_info {
		...
		border-radius: 30upx;
		margin: 30upx auto;
		background: url(../../static/img/activity.png) no-repeat;
